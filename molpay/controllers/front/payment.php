...
class MOLPayPaymentModuleFrontController extends ModuleFrontController
{
	...
	public function initContent()
	{
		...
		$molpayparams['mpsbill_desc']		= "";
		$molpayparams['mpsbill_desc']		.="----------------------------------\nBilling Address\n----------------------------------\n";
		$molpayparams['mpsbill_desc']		.=$address->address1." ".$address->address2." ".$address->postcode." ".$address->city." ".$address->country;
		$molpayparams['mpsbill_desc']		.="\n----------------------------------\nProduct(s) Info\n----------------------------------\n";
		for($i=0; $i<count($prod_obj); $i++) {
			$molpayparams['mpsbill_desc']	.=$prod_obj[$i]['name']." x ".$prod_obj[$i]['cart_quantity'];
			$molpayparams['mpsbill_desc']	.="\n";
		}
